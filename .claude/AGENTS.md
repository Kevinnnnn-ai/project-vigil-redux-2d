...
A 2D reinforcement-learning sandbox in which a from-scratch PPO agent learns to fly and land a single-stage, gimbaled rocket booster inside a Pymunk (Chipmunk2D) rigid-body simulation—landing, settling, and tip-over emerge from the physics solver, not from a scripted verdict.
